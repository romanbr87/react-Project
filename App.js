...
function App() {
  ...
  const [rooms, setRooms] = useState([
    { color: "rgb(0,255,20)", name: "Soute", roomType: "Bathroom", products: [
 ...
 //  Get's room and update Info  
  ...
  const addRoom = (name, color, roomType) => {
    ...
    setRooms([{ name: name, color: color, roomType: roomType, products: [] }, ...rooms])
  }
  ...
  //  filter: Only 1 stereo in a room
    let streo = room.products.filter(product => product.name == "Stereophonic sound")
  ...
  // copy the variable and gives a new one instead
    let myRooms = Object.create(rooms)
     myRooms[room.index].products[deviceIndex].isTurnedOn = !myRooms[room.index].products[deviceIndex].isTurnedOn
     setRooms (myRooms)
     getRoom (room.index)
  }
  ...
  return (
    <div className='App'>
      <Router>
        <Link to="/"><h1 style={{ backgroundColor: 'rgb(84, 166, 189, 0.68)',padding:'5px', border: '5px solid #ffdedeb3', borderRadius:'10px', display: "inline-flex" }}>My Smart Home</h1></Link>
        <Switch>
          <Route exact path="/" component={() => { return <HomePage rooms={rooms} getRoom={getRoom} del={removeRoom} /> }} />
          <Route exact path='/AddRoom' component={() => { return <AddRoom add={addRoom} /> }} />
          <Route exact path='/Room' component={() => { return <Room room={room} addDevice={addDevice} swithchDevice={swithchDevice}/> }} />
        </Switch>
      </Router>
    </div>
  );
